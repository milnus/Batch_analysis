...
tidy_growth_format <- function(growth_data, bootstaps = 1) {
  ...
  for (description in names(growth_data)) {
    ...
    growth_data_list[[description]] <- data.frame(
      "mu" = tt_growth_rate[[3]]$gcFittedSplines[[1]]$parameters$mu,
    ...
    # Add spline data
    spline_data_list[[description]] <- data.frame(
      "Time" = tt_growth_rate$gcFit$gcFittedSplines[[1]]$fit.time,
